...
_drawMarkers = {
	...
	{if ((waypointType _x)=="MOVE") then {
		...
		_0lb = str _forEachIndex + " " + str (waypointType _x);
		[_markerName,_pos,_0lb] call _addMarker;
...
case 0: {//init;
		_target_mode = if isNil{_opt} then {//restore values
			...
			PG_set(target_mode,_opt);
			{
				_05b = _x select 0;
				deleteVehicle _05b;
				if !(_05b isKindOf "Man") then {{deleteVehicle _x} forEach units (_x select 2);};
...
case 1: {
		...
		for "_i" from 0 to (count _cfgvehicles)-1 do {
			...
			if (isClass _0f) then {
				_opt = configName(_0f);
				if ((getNumber(_0f >> "scope")==2)and(KINDOF_ARRAY(_opt,_kindOf))and!(KINDOF_ARRAY(_opt,_filter))) then {
					GET_CTRL(balca_target_vehlist_IDC) lbAdd (getText(_0f >> "displayName"));
					...
					if !(_kindOf select 0 in ["TargetBase","CAManBase"]) then {
						GET_CTRL(balca_target_vehlist_IDC) lbSetPicture [(lbSize GET_CTRL(balca_target_vehlist_IDC))-1,getText(_0f >> "picture")];
					}else{
						GET_CTRL(balca_target_vehlist_IDC) lbSetPicture [(lbSize GET_CTRL(balca_target_vehlist_IDC))-1,getText(_0f >> "icon")];
					};
...
case 2: {//info
		...
		_0f = (configFile >> "CfgVehicles" >> _opt);
		...
		GET_CTRL(balca_target_veh_info_IDC) ctrlSetStructuredText _0lb;
	};
case 3: {//add target
		...
		switch PG_get(target_mode) do {
			case 0: {//land static
				...
				if ((PG_get(TARGET_PROPS) select 1)>0) then {//if speed>0 start moving
					[] spawn {
						_0tn = 0;
						_delay = 0.03;
						_0ra = (PG_get(TARGET_PROPS) select 1);
						_shift_inc = (_0ra*_delay);
						while {((PG_get(TARGET_PROPS) select 1) == _0ra)&&(PG_get(target_mode) in [0,3])} do {
							sleep _delay;
							_0tn = _0tn + _shift_inc;
							if (abs(_0tn)>20) then {_shift_inc = -_shift_inc};
							[_0tn,-1] call PG_get(fnc_move_land_targets);
						};
			...
			case 2: {//land AI
				...
				if ((ctrlVisible balca_target_map_IDC)&&((count waypoints PG_get(TARGET_GRP))>1)) then {
					...
					_wp = _grp addWaypoint [[(_pos select 0)+20*sin(_0xn+90)+(_props select 0)*sin(_0xn),(_pos select 1)+20*cos(_0xn+90)+(_props select 0)*cos(_0xn)],0,0];
					_wp setWaypointType "MOVE";
					_wp setWaypointCompletionRadius 30;
					_wp = _grp addWaypoint [[(_pos select 0)-20*sin(_0xn+90)+(_props select 0)*sin(_0xn),(_pos select 1)-20*cos(_0xn+90)+(_props select 0)*cos(_0xn)],0,1];
					_wp setWaypointType "MOVE";
					_wp setWaypointCompletionRadius 30;
...
case 9: {//add waypoint
		...
		_04b = (count (waypoints _grp)) max 1;
		deleteWaypoint [_grp,_04b-1];
		_wp = _grp addWaypoint [_opt,0,_04b-1];
		_wp setWaypointType "MOVE";
		_wp setWaypointCompletionRadius 30;
		if (ctrlVisible balca_target_map_IDC) then {
			_markerName = format ["PG_WPMarker%1",str (_04b-1)];
			_pos = waypointPosition _wp;
			_0lb = str (_04b-1) + " " + str (waypointType _wp);
			[_markerName,_pos,_0lb] call _addMarker
		};
		_wp = _grp addWaypoint [_opt,0,_04b];
		_wp setWaypointType "CYCLE";
	};
case 10: {//unload
		[] call _clearMarkers;
