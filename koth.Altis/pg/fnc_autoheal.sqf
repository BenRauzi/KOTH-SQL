...
if PG_get(Autoheal) then {
	...
	[] spawn {
		while {PG_get(Autoheal)} do {
			...
			_veh_hit_EH = _05d addEventHandler ["hit",{vehicle player setDammage 0}];
			_veh_dam_EH = _05d addEventHandler ["dammaged",{vehicle player setDammage 0}];
			while {(_05d == (vehicle player))&&PG_get(Autoheal)} do {
			...
			_05d removeEventHandler ["hit",_veh_hit_EH];
			_05d removeEventHandler ["dammaged",_veh_dam_EH];
		};
	};
};